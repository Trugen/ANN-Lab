...
SnapShots="../snapshots/hvvan_R1_cityscape_"
...
do
srun --gres=gpu:1 --pty python ../train.py --restore-from $restore_from --start $[$StartIter+$SaveEvery*$[$Iter-1]] --max-iter $TotalIter --random-mirror --random-scale --learning-rate 1e-2 --input-size 512 --weight-decay 1e-4 --batch-size 8 --num-steps $SaveEvery --save-pred-every $SaveEvery --recurrence 1 --model hvvan --van-size van_large --dataset cityscape --num-classes 19
